      ...
      if (didScroll && firing === true) {
        ...
        if (scrollTarget == document || scrollTarget == window) {
          ...
          if (inner_wrap.length == 0) {
            inner_wrap = $(scrollTarget).wrapInner("<li class=\"endless_scroll_inner_wrap\" />").find(".endless_scroll_inner_wrap");
          }
        ...
        if (is_scrollable && (options.fireOnce == false || (options.fireOnce == true && fired != true))) {
          ...
          $(options.insertAfter).after("<li class=\"endless_scroll_loader_" + scrollId + " endless_scroll_loader\">" + options.loader + "</li>");
	  ...
	  if(options.url !== false){
		...
		success: function(data) {
			  ...
			  if (options.fireDelay !== false || options.fireDelay !== 0) {
				$("body").after("<li id=\"endless_scroll_marker\"></li>");
			...
			$(options.insertAfter).after("<span class='over'>已经加载完所有的数据</span>");
			--p;
		      }
